...
class Fit:
    ...
    def lnprior(self, theta, bound_penalty=False):
        ...
        if "lnprior" in self.debug:
            print(
                "debug Fit.lnprior: s, q, rho, u0, alpha, t0, tE, piEE, piEN, i, phase, period: "
            )
            print("                   ", theta)
        ...
        if (
            ...
            if (
                bound_penalty
            ):  # i'm not using this. I need to redo the calculation
            ...
            if "lnprior" in self.debug:
                ...
                if period / tE < 4:
                    print(
                        "debug Fit.lnprior: period/tE = ", period / tE, " < 4"
                    )
                if s < 0.001:
