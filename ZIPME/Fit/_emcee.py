...
def run_emcee(self, nl, ndim, stepi, mi, log_prob_function, state, event_obj, threads=1, event_name='', path='./', labels=None):
    if threads > 1:
        with Pool(threads) as pool:
            # Initialize the sampler
            sampler = emcee.EnsembleSampler(nl, ndim, log_prob_function, args=[event_obj], pool=pool)
        ...
        # Initialize the sampler
        sampler = emcee.EnsembleSampler(nl, ndim, log_prob_function, args=[event_obj])
