...
class Event:
    ...
    def projected_seperation(self, i, period, t, t_start, phase_offset=0.0, a=1.0):
    ...
    def get_magnification(self, t, obs=None):
        ...
        t_start = self.data.sim_time0
        tE = self.params[6].copy()  # Einstein crossing time/event timescale
        tau_start = (t_start-t0)/tE
        tcroin = self.true_params['tcroin']  # the real t0

        #
        sc, _, _ = self.projected_seperation(i, period, tcroin, t_start, phase0) # star-centric ref frame
        a = s/sc  # semimajor axis in uits of thetaE
        ...
        ss1, x1, y1 = self.projected_seperation(i, period/tE, tau, tau_start, phase0+np.pi, a1)  # star
        ss2, x2, y2 = self.projected_seperation(i, period/tE, tau, tau_start, phase0, a2)  # planet
        ...
        # - s1 moves to CoM
        xsin = tt * cosalpha - uu * sinalpha - s1
        self.traj_parallax_u1[obs] = xsin
        self.traj_base_u1[obs] = tau * cosalpha - beta * sinalpha - s1
