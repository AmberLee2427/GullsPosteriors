...
class Orbit:
    ...
    def __init__(self, 
                 ...
                 start_date='2018-08-10', #2018-04-25
                 end_date='2023-05-30', 
                 refplane='ecliptic', 
                 n_epochs=None,
                 origin='500@10',
...
class Event:
    ...
    def get_magnification(self, t, obs=None, p0=False):
        ...
        t_ref = self.t_ref.copy()  # reference time for the parallax
        ...
        self.traj_base_u1[obs] = tau * cosalpha - beta * sinalpha - self.truths['xCoM']  # sans parallax, for debug
...
if __name__ == '__main__':
    ...
    for i in range (nevents):
        ...
        np.savetxt(path+'posteriors/'+event_name+'_chi2.txt', np.array([chi2_t0, chi2_tc]))
        ...
        plt.axvline(x=event_tc.t_ref, color='cyan', linestyle='--', label=r'$t_{ref}$', alpha=0.5)

        
        plt.xlabel('BJD')
        xmin = np.min([t0-1.0*tE, event_tc.t_ref-2.0])
        xmax = np.max([t0+1.0*tE, event_tc.t_ref+2.0])
        plt.xlim(xmin, xmax)
        ...
        y0 = u0 * np.cos(alpha)
        x0_ref = -u0 * np.sin(alpha) + (event_tc.t_ref-t0)/tE * np.cos(alpha) - xcom
        y0_ref = u0 * np.cos(alpha) + (event_tc.t_ref-t0)/tE * np.sin(alpha)
        ...
        plt.axvline(x=(event_tc.t_ref-t0)/tE, color='cyan', linestyle='--', label=r'$t_{ref}$', alpha=0.5)
