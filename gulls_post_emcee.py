...
class Fit:
    ...
    def prior_transform(self, u, true, prange, normal=False):
        ...
        x = np.array([s, q, rho, u0, alpha, t0, tE, piEE, piEN, i, phase, period])
    ...
    def plot_chain(self, res, event_name, path, labels=None, pt_args=[]):
        samples = res.get_chain()
        samples = self.prior_transform(samples, *pt_args)
...
if __name__ == '__main__':
    ...
    for i in range (nevents):
        ...
        initial_state = fit.prior_transform(initial_pos, truths['params'], bounds, normal)
        print('Debug main: state = ', initial_state)
        ...
        sampler = emcee.EnsembleSampler(nl, ndim, fit.lnprob_transform, args=[event_fit, truths['params'], bounds, normal])

        # Run the MCMC
        steps = 0
        pos = initial_pos.copy()
        while steps < mi:
            pos, lnp, _ = sampler.run_mcmc(pos, stepi, progress=True)
            chain = sampler.chain
            flatchain = sampler.flatchain
            lnprobability = sampler.lnprobability
            flatlnprobability = sampler.flatlnprobability
            state = fit.prior_transform(pos, truths['params'], bounds, normal)
            ...
            # Save the samples
            samples = fit.prior_transform(chain, truths['params'], bounds, normal=normal)
            np.save(path+'posteriors/'+event_name+'_emcee_samples.npy', samples)
            np.save(path+'posteriors/'+event_name+'_emcee_lnprob.npy', lnprobability)
            np.save(path+'posteriors/'+event_name+'_emcee_state_u.npy', pos)
            np.save(path+'posteriors/'+event_name+'_emcee_state.npy', state)

            fit.plot_chain(sampler, event_name, path, labels=labels, pt_arg=[truths['params'], bounds, normal])
        ...
        res = sampler.results
        res.samples = fit.prior_transform(res.samples, truths['params'], prange, normal=normal)
