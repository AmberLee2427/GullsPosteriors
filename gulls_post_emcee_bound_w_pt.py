...
if __name__ == '__main__':
    ...
    if LOM_enabled:
        ...
        linear_indices = [3, 4, 5, 6, 7, 8, 9, 10] # u0, alpha, t0, tE, piEE, piEN, i, phase
        ...
        linear_indices = [3, 4, 5, 6, 7, 8] # u0, alpha, t0, tE, piEE, piEN
        p_unc_linear_space = p_unc[linear_indices]
        prange_linear = p_unc_linear_space * 2.0
    # ==================================================================
    ...
    # --- Initializations ---
    orbit_obj = Orbit()
    fit_obj = Fit(sampling_package=sampling_package, LOM_enabled=LOM_enabled, ndim=ndim, labels=labels)
    vbm = VBMicrolensing()
    vbm.a1 = 0.36
    ...
    for i in range (nevents):
        ...
        chi2_ew_t0_true = ( (f_true[0] - (event_t0.get_magnification(t_data[0],0)*fit_obj.get_fluxes(event_t0.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[0] + fit_obj.get_fluxes(event_t0.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[1]))**2 / f_err_true[0]**2 )[points]
        chi2_ew_tc_true = ( (f_true[0] - (event_tc.get_magnification(t_data[0],0)*fit_obj.get_fluxes(event_tc.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[0] + fit_obj.get_fluxes(event_tc.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[1]))**2 / f_err_true[0]**2 )[points]
        chi2_ew_tref_true = ( (f_true[0] - (event_tref.get_magnification(t_data[0],0)*fit_obj.get_fluxes(event_tref.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[0] + fit_obj.get_fluxes(event_tref.get_magnification(t_data[0],0), f_true[0], f_err_true[0]**2)[1]))**2 / f_err_true[0]**2 )[points]
        
        chi2_list = [np.sum(chi2_ew_t0_true), np.sum(chi2_ew_tc_true), np.sum(chi2_ew_tref_true)]
        ...
        if plot_initial_figures:
            ...
            for obs in event_tc.data.keys():
                ...
                ax1.plot(t_data[obs], (data[obs][1]-fb)/fs, '.', color=colours[obs], label=data_labels[obs], alpha=0.5, zorder=0)

            ax1.plot(tt, event_tc.get_magnification(tt, 0), '-', color='cyan', label=f'$t_c$={event_tc.t_ref:.1f}', zorder=6, alpha=0.75, lw=1)
            ax1.plot(tt, event_t0.get_magnification(tt, 0), '-', color='blue', label=f'$t_0$={event_t0.t_ref:.1f}', zorder=4, alpha=0.75, lw=1)
            ax1.plot(tt, event_tref.get_magnification(tt, 0), '-', color='purple', label=f'$t_c,calc$={event_tref.t_ref:.1f}', zorder=2, alpha=0.75, lw=1)
            ax1.axvline(x=fit_tref, color='orange', linestyle='-', alpha=0.25, zorder=0, linewidth=4)
            
            ax1.set_xlim(tmin, tmax)
            ax1.set_ylabel('Magnification')
            if LOM_enabled:
                title_str = 's=%.2f, q=%.6f, rho=%.6f, u0=%.2f, alpha=%.2f, t0=%.2f, \ntE=%.2f, piEE=%.2f, piEN=%.2f, i=%.2f, phase=%.2f, period=%.2f'
            else:
            ...
            if LOM_enabled:
                # Caustic Plot
                plt.figure()
                s_tc, _, _ = event_tc.projected_seperation(truths['params'][9], truths['params'][11], truths['tcroin'], phase_offset=truths['params'][10], t_start=truths['tcroin'], a=truths['Planet_semimajoraxis']/truths['rE'])
                caustics_tc = vbm.Caustics(s_tc, truths['params'][1])
                for closed in caustics_tc:
                    plt.plot(closed[0], closed[1], '-', color='cyan', ms=0.2, zorder=0, alpha=0.5)
                plt.plot(event_tc.lens1_0[0], event_tc.lens1_0[1], 'o', ms=10, color='red', zorder=0)
                plt.plot(event_tc.lens2_0[0], event_tc.lens2_0[1], 'o', ms=10**q, color='red', zorder=0)
                plt.plot(event_tc.traj_parallax_dalpha_u1[0][points], event_tc.traj_parallax_dalpha_u2[0][points], '-', color='cyan', alpha=0.5)
                ...
                plt.xlabel(r'$\tau$'); plt.ylabel('s'); plt.legend()
                ...
                plt.xlabel(r'$\tau$'); plt.ylabel(r'd$\alpha$'); plt.legend()
                plt.savefig(path+'posteriors/'+event_name+'_dalphadtau.png')
                plt.close()

        end_initial_figures = time.time()
        print('Time to make initial figures = ', end_initial_figures - end_preamble)
        ...
        for obs_key in data.keys():
            ...
            # Calculate cropping indices specifically for this observatory's time data
            points_for_this_obs = np.where(np.logical_and(current_obs_t_data > tmin_fit, current_obs_t_data < tmax_fit))
        ...
        sampler = fit_obj.run_emcee(nl, ndim, stepi, mi,
                                    ...
                                    prange_linear, prange_log, normal,
                                    threads=threads, event_name=event_name, path=path, labels=labels)
        ...
        samples_phys = fit_obj.prior_transform(flat_chain, truths['params'][:ndim], prange_linear, prange_log, normal=True)
        np.save(path + 'posteriors/' + event_name + '_post_samples.npy', samples_phys)
        ...
        if plot_trace:
            fit_obj.traceplot(sampler, event_name, path, truths) # Assuming traceplot exists
        if plot_run:
            fit_obj.runplot(sampler, event_name, path) # Assuming runplot exists
        ...
        if plot_final_figures:
            ...
            ax1.set_ylabel('Magnification'); ax2.set_ylabel('Residuals'); ax2.set_xlabel('BJD')
