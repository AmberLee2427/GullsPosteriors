    ...
    def load_data(self, data_file):
        ...
        for code in data['observatory_code'].unique():
            data_obs = data[data['observatory_code'] == code][['BJD', 
                                          ...
                                          'parallax_shift_u',
                                          'true_relative_flux', 
                                          'true_relative_flux_error']].reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    for i in range (nevents):
        ...
        chi2_t0_0 = np.sum(chi2_ew_t0[0])
        cumsum_chi2_t0 = np.cumsum(chi2_ew_t0[0])
        chi2_tc_0 = np.sum(chi2_ew_tc[0])
        cumsum_chi2_tc = np.cumsum(chi2_ew_tc[0])
        chi2_tref_0 = np.sum(chi2_ew_tref[0])
        ...
        for obs in event_tc.data.keys():
            ...
            A_true[obs] = (f_true[obs] - fb[obs])/fs[obs]
            if int(obs) == 0:
                res['tc'] = A_true[obs] - A[obs]
                res['t0'] = event_t0.get_magnification(t, obs) - A[obs]
                res['tref'] = event_tref.get_magnification(t, obs) - A[obs]
                ax2.plot(t, res['tc'], '.', color='cyan', alpha=0.35, zorder=0, ms=1.5, label=r'$\chi^2=%1.0f$' %chi2_tc)
                ax2.plot(t, res['t0'], '.', color='blue', alpha=0.35, zorder=0, ms=1.5, label=r'$\chi^2=%1.0f$' %chi2_t0)
                ax2.plot(t, res['tref'], '.', color='purple', alpha=0.35, zorder=0, ms=2, label=r'$\chi^2=%1.0f$' %chi2_tref)
        ...
        ax1.plot(tt, A_lin, 
                 '-', 
                 label=r'$t_{ref}=t_c=%1.2f$' %event_tc.t_ref,
                 zorder=6,
                 color='cyan', 
                 alpha=0.75, 
                 lw=1
        ...
        ax1.plot(tt, A_lin, 
                '-', 
                label=r'$t_{ref}=t_0=%1.2f$' %event_t0.t_ref,
                zorder=4, 
                color='blue', 
                alpha=0.75, 
                lw=1
        ...
        ax1.plot(tt, A_lin, 
                '-', 
                label=r'$t_{ref}=t_{c,calc}=%1.2f$' %event_tref.t_ref, 
                zorder=2, 
                color='purple', 
                alpha=0.75, 
                lw=1
