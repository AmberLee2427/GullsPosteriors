...
class Event:
    ...
    def tref2t0(self, truths):
        ...
        ''' pretty sure this is all wring because u0 is actually defined at
        time t_ref. 
        It would make more sense to convert to COM and then from t_ref to t0
        ...
        if self.t_ref != t0:
            ...
            truths['xCoM'] = s * q/(1.0+q) * s

            # redefining s to be at time t0 instead of tref
            S_ref, Sx_ref, Sy_ref = self.projected_seperation(i, period, self.t_ref, phase_offset=phase0, t_start=self.t_ref)
            S_t0, Sx_t0, Sy_t0 = self.projected_seperation(i, period, t0, phase_offset=phase0, t_start=self.t_ref)  
            print('\ndebug Event.tref2t0: s_ref (dimensionless): ', S_ref)
            print('debug Event.tref2t0: s_t0  (dimensionless): ', S_t0)
            print('debug Event.tref2t0: s_gulls (t=tref): ', s)
            s_t0 = s*S_t0/S_ref  # S_t0 and S_ref are in nonsense units, but the ratio is valid
            print('debug Event.tref2t0: s_new (t=t0): ', s)

            delta_x0_com = s_t0 * q/(1.0+q) * s_t0
            truths['xCOM'] = delta_x0_com
            print('\ndebug Event.tref2t0: delta_x0_com: ', delta_x0_com)

            # I think alpha is defined at tcroin 
            #'''
            # rotation around com due to orbital motion between t0 and tref
            rot_ref_com = np.arctan2(Sy_ref, Sx_ref)  # again, the size of Sx and Sy are nonsense, but the ratio is valid
            rot_t0_com = np.arctan2(Sy_t0, Sx_t0)
            print('\ndebug Event.tref2t0: rot_ref_com: ', rot_ref_com)
            print('debug Event.tref2t0: rot_t0_com: ', rot_t0_com)
            rot_com = rot_t0_com - rot_ref_com
            print('debug Event.tref2t0: rot_com: ', rot_com)
            ...
            x0_com_rot = x0_com * np.cos(rot_com) - y0_com * np.sin(rot_com)
            y0_com_rot = x0_com * np.sin(rot_com) + y0_com * np.cos(rot_com)
            print('\ndebug Event.tref2t0: alpha_gulls: ', alpha)
            alpha_com = np.arctan2(y0_com_rot, x0_com_rot) # alpha changes because t0 changes
            print('debug Event.tref2t0: alpha_t0_com: ', alpha_com)
            phase0_t0_com = phase0 + (t0_com - self.t_ref) * 2.0 * np.pi / period

            truths['params_t0_L1'] = [s_t0, q, rho, u0, alpha, t0, tE, piEE, piEN, i, phase0_t0_L1, period]
            truths['params_t0_COM'] = [s_t0, q, rho, u0_com, alpha_com, t0_com, tE, piEE, piEN, i, phase0_t0_com, period]
    ...
    def get_magnification(self, t, obs=None, p0=False):
        ...
        #
        S_ref, _, _ = self.projected_seperation(i, period, self.t_ref, phase_offset=phase0) # star-centric ref frame
        if p0:
            s_ref = self.params[0].copy()
        else:
            s_t0 = self.params[0].copy()
            S_t0, _, _ = self.projected_seperation(i, period, t0, phase_offset=phase0) # star-centric ref frame
            #s_t0 = s_ref*S_t0/S_ref
            s_ref = s_t0 * S_ref / S_t0
        a = s_ref/S_ref  # semimajor axis in uits of thetaE
        print('debug Event.get_magnification: a:', self.truths['Planet_semimajoraxis'], a)
        ...
        # this coordinate system is ddefined at time t_ref with a COM origin
        ...
        # - s1 moves to CoM
        xsin = tt * cosalpha - uu * sinalpha - self.truths['xCoM']
        self.traj_parallax_u1[obs] = xsin
        self.traj_base_u1[obs] = tau * cosalpha - beta * sinalpha - self.truths['xCoM']
        ...
        rot = np.arctan2(y2, x2)  # position of the planet in the COM frame
                                  # what is the reference time for this?
        _, x0, y0 = self.projected_seperation(i, period, t_start, t_start, phase0) # star-centric ref frame
        rot0 = np.arctan2(y0, x0)  # at reference time; tcroin
...
if __name__ == '__main__':
    ...
    for i in range (nevents):
        ...
        event = Event(parallax, orbit, data, truths, data_structure.sim_time0, t_ref=truths['tcroin'])
