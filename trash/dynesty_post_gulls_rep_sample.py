...
for i in range (nevents):
    true_params, ra, dec, data = new_event(sort)
    ...
    model = mm.Model(p_dic)
    
    event = mm.Event(datasets=data, model=model)
    sate = mm.satelliteskycoord.SatelliteSkyCoord(ephemerides_file, satellite='Roman')
    coords = astrocoords.SkyCoord(ra=ra, dec=dec, unit=(u.deg, u.deg), frame='icrs', obstime='J2000')
    mm.trajectory.Trajectory(t, event.model.parameters, parallax={'satellite': True}, coords=coords, satellite_skycoord=sate, earth_coords=None)
    
    
    mm.set_limb_coeff_gamma('W146', 0.36)
    mm.set_limb_coeff_gamma('Z087', 0.36)
    mm.set_limb_coeff_gamma('K213', 0.36)
    mm.set_magnification_methods(['VBBL'])
