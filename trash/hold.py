        ...
        if start > 0 or start == -1:
            ...
            if "PERMUTE_PAIRS" in special:
                for obsidx in range(paramfile.numobservatories):
                    for obsjdx in range(
                        obsidx + 1, paramfile.numobservatories
                    ):
                        pair = [obsidx, obsjdx]
            ...
            if "PERMUTE_REMOVE" in special:
                for obsidx in range(paramfile.numobservatories - 1, -1, -1):
                    set_ = [
                        obsjdx
                        for obsjdx in range(paramfile.numobservatories)
                        if obsjdx != obsidx
                    ]
            ...
            while elstart != -1:
                ...
                if "ALL" in elstr:
                    ...
                    if 0 <= candidate < paramfile.numobservatories:
                        ...
                        print(
                            f"Invalid observatory number in group {len(event.obsgroups) - 1} ({elstr})"
                        )
                        exit(1)
    ...
    for obsidx in range(paramfile.numobservatories):
        ...
        if len(jdepochs) > 0:
            ...
            event.pllx[obsidx].set_reference(
                paramfile.simulation_zerotime + tref, world[0].orbit
            )
            ...
            if paramfile.verbosity > 3:
                for idx in range(len(world[obsidx].jd)):
                    print(
                        f"NEShift event {event.id} obs {obsidx} {world[obsidx].jd[idx]:14.6f} {event.pllx[obsidx].NEshift[idx][0] + (event.pllx[obsidx].epochs[idx] - event.pllx[obsidx].tref) * event.pllx[obsidx].vref[0]} {event.pllx[obsidx].NEshift[idx][1] + (event.pllx[obsidx].epochs[idx] - event.pllx[obsidx].tref) * event.pllx[obsidx].vref[1]}"
                    )
        ...
        if paramfile.verbosity > 0:
            ...
            print("Orbit:")
            print(
                f"{world[obsidx].orbit[0].xh} {world[obsidx].orbit[0].yh} {world[obsidx].orbit[0].zh}"
            )
            print(f"tref = {event.pllx[obsidx].tref}")
            print(
                f"l,b,a,d = {event.pllx[obsidx].l} {event.pllx[obsidx].b} {event.pllx[obsidx].a} {event.pllx[obsidx].d}"
            )
            print(
                f"mul, mub, mua, mud = {event.pllx[obsidx].mul} {event.pllx[obsidx].mub} {event.pllx[obsidx].mua} {event.pllx[obsidx].mud}"
            )
            print(
                f"piEN, piEE, piEll, piErp, piE = {event.pllx[obsidx].piEN} {event.pllx[obsidx].piEE} {event.pllx[obsidx].piEll} {event.pllx[obsidx].piErp} {event.pllx[obsidx].piE}"
            )
            print(
                f"tE_h, tE_r = {event.pllx[obsidx].tE_h} {event.pllx[obsidx].tE_r}"
            )
...
class Parallax:
    ...
    def compute_NEshifts(self):
        if not (
            self.status_refframe
            and self.status_orbit
            and self.status_position
            and self.status_sourcedir
            and self.status_parallax
            and self.status_params
            and self.status_epochs
        ):
            print(
                f"{__file__}: {__name__}: Error: Some element of the parallax computation is uninitialized:"
            )
            self.print_uninit()
            exit(1)
        ...
        for i in range(len(self.epochs)):
            ...
            self.NEshift[i][0] = (
                x[0]
                - self.xref[0]
                - (self.epochs[i] - self.tref) * self.vref[0]
            )
            self.NEshift[i][1] = (
                x[1]
                - self.xref[1]
                - (self.epochs[i] - self.tref) * self.vref[1]
            )
    ...
    def compute_tushifts(self):
        ...
        for i in range(len(self.epochs)):
            self.tushift[i][0] = -self.piE * (
                self.NEshift[i][0] * cs + self.NEshift[i][1] * sn
            )
            self.tushift[i][1] = -self.piE * (
                -self.NEshift[i][0] * sn + self.NEshift[i][1] * cs
            )
    ...
    def compute_directions(self):
        ...
        if not self.status_orbit:
            print(
                f"{__file__}: Error: Orbit not set (either through piE components or proper motions)."
            )
            exitnow = True
        ...
        if not self.status_sourcedir:
            print(
                f"{__file__}: Error: Source direction not set (either through piE components or proper motions)."
            )
            exitnow = True
        ...
        if self.progress[LPARAMS] == 1 and self.progress[LSOURCEDIR] == 1:
            ...
            self.piEll = self.piEN * np.cos(
                -self.phi_llN
            ) - self.piEE * np.sin(-self.phi_llN)
            self.piErp = self.piEN * np.sin(
                -self.phi_llN
            ) + self.piEE * np.cos(-self.phi_llN)
            ...
            if self.progress[LSOURCEDIR] == 2:
                ...
                self.piEN = self.piEll * np.cos(
                    self.phi_llN
                ) - self.piErp * np.sin(self.phi_llN)
                self.piEE = self.piEll * np.sin(
                    self.phi_llN
                ) + self.piErp * np.cos(self.phi_llN)
            wr = np.zeros(2)
            wh = np.zeros(2)
            ...
            print(
                f"{__file__}: {__name__}: Error: Must specify parameters and source directions as heliocentric or reference-frame-centric"
            )
    ...
    def set_radec(self, ra, dec, deghr):
        if deghr > 2:
            print(
                f"{__file__}: {__name__}: Error setting position - bad angle unit code ({deghr})"
            )
            exit(1)
    ...
    def set_radec_sexagesimal(self, ra, dec):
        if len(ra) != 3 or len(dec) != 3:
            print(
                f"{__file__}: {__name__}: Error setting position - input sexagesimal vectors (ra_hr, ra_min, ra_sec), (dec_deg, dec_min, dec_sec)"
            )
            exit(1)
        ...
        a_ = np.deg2rad(
            (ra[0] + sgn_a * (ra[1] / 60.0 + ra[2] / 3600.0)) * 15.0
        )
    ...
    def set_lb(self, l_, b_, deg):
        if deg > 1:
            print(
                f"{__file__}: {__name__}: Error setting position - bad angle unit code ({deg})"
            )
            exit(1)
    ...
    def set_pm_lb(self, mul_, mub_):
        if not self.status_position:
            print(
                f"{__file__}: {__name__}: Must set position in order to do proper motion unit conversions"
            )
            exit(1)
